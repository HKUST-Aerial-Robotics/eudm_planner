...
class EudmPlannerMapItf {
 public:
  ...
  virtual ErrorType GetUncertainVehicleIds(
      std::vector<int> *uncertain_vehicle_ids) = 0;
  ...
  virtual ErrorType LateralBehaviorPredictionUsingMobil(
      const common::Vehicle &vehicle, const common::VehicleSet &nearby_vehicles,
      std::set<std::pair<decimal_t, LateralBehavior>> *res) = 0;
};
