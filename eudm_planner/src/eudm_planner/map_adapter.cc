...
ErrorType EudmPlannerMapAdapter::GetUncertainVehicleIds(
    std::vector<int> *uncertain_vehicle_ids) {
  if (!is_valid_) return kWrongStatus;
  *uncertain_vehicle_ids = map_->uncertain_vehicle_ids();
  return kSuccess;
}
...
ErrorType EudmPlannerMapAdapter::GetSpeedLimit(const State &state,
                                               const Lane &lane,
...
ErrorType EudmPlannerMapAdapter::LateralBehaviorPredictionUsingMobil(
    const common::Vehicle &vehicle, const common::VehicleSet &nearby_vehicles,
    std::set<std::pair<decimal_t, LateralBehavior>> *res) {
  if (!is_valid_) return kWrongStatus;
  if (map_->MobilRuleBasedBehaviorPrediction(vehicle, nearby_vehicles, res) !=
      kSuccess) {
    return kWrongStatus;
  }
  return kSuccess;
